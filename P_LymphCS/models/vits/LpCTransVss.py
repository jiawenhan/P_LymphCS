...
from P_LymphCS.models.vmamba.vmamba import *
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.vssm = VSSBlock(hidden_dim=dim, ssm_d_state=16, ssm_conv_bias=False, patch_norm=True)
    ...
    def forward(self, x, H, W):
        B, N, C = x.shape
        x_norm = self.norm1(x).permute(0, 2, 1).reshape(B, C, H, W)
        x_vssm = self.vssm(x_norm)
        x_vssm = x_vssm.flatten(2).transpose(1, 2)
        x = x + self.drop_path(x_vssm)
        x = x + self.drop_path(self.mlp(self.norm2(x)))       
        # x = x + self.drop_path(self.vssm(self.norm1(x), H, W))
        # x = x + self.drop_path(self.mlp(self.norm2(x)))
        return x
...
class GroupBlock(Block):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        super(GroupBlock, self).__init__(dim, num_heads, mlp_ratio, qkv_bias, qk_scale, drop, attn_drop,
                                         drop_path, act_layer, norm_layer, sr_ratio, ws)
    ...
    def forward(self, x, H, W):
        if self.ws == 1:
            ...
            x_spatial = x.reshape(B, H, W, C).contiguous()
            x_norm = self.norm1(x_spatial) 
            x_vssm = self.vssm(x_norm) 
            x_vssm = x_vssm.reshape(B, N, C).contiguous()
...
class LpCTransVss(PCPVT):
    """
    alias Twins+VSSM
    """
    def __init__(self, img_size=512, patch_size=4, in_chans=3, num_classes=5, embed_dims=[128, 256, 512, 1024],
                 num_heads=[4, 8, 16, 32], mlp_ratios=[4, 4, 4, 4], qkv_bias=True, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[2, 2, 18, 2], sr_ratios=[8, 4, 2, 1], block_cls=GroupBlock, wss=[8, 8, 8, 8]):
        super(LpCTransVss, self).__init__(img_size, patch_size, in_chans, num_classes, embed_dims, num_heads,
                                     mlp_ratios, qkv_bias, qk_scale, drop_rate, attn_drop_rate, drop_path_rate,
                                     norm_layer, depths, sr_ratios, block_cls)
        del self.blocks
        self.wss = wss
...
def P_LymphCS(pretrained=False, **kwargs):
    model = LpCTransVss()
    return model