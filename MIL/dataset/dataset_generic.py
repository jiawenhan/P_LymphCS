...
class Generic_MIL_Dataset(Generic_WSI_Classification_Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            if not self.use_h5:
                ...
                if full_path in self.data_cache.keys():
                    ...
                    try:
                        ...
                        print(f"Failed to load PT feature file: {full_path}")
                        print(f"Error: {e}")
                        self.corrupted_files.append(full_path)
                        raise

                    if getattr(self, 'cache_flag', False):
                ...
                try:
                    ...
                    print(f"Failed to load H5 feature file: {full_path}")
                    print(f"Error: {e}")
                    self.corrupted_files.append(full_path)
                    raise
            ...
            print(f"Unable to load features for slide_id={slide_id}")
            traceback.print_exc()
            raise RuntimeError(
                f"Feature loading failed for slide_id={slide_id}"
            ) from e
    ...
    def get_corrupted_files(self):
        """Return the unique paths of feature files that failed to load."""
        return list(set(self.corrupted_files))
